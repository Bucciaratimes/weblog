...
正直あれ以降も思うことはあって、データ的にこれが感覚的なものなのか、現実的な問題なのか確かめたいなと思いました。
...
以上の説明を踏まえて「PSxG-失点数(90分あたり)」の値をみていきます。下記のようになってました。（上図の数字では＋50してるので-50してます）。
...
そういう意味で、この「-0.12」という数字はちょっとマイナス過ぎるのです。この場合、失点になってるシュートのセーブ難易度が平均的もしくは平均以下と考えるのが妥当なのかな？と思いました。
...
ここまで「テアの不調って現実として起きてることなの？」→「起きてるくさい」、「ではいつまで続きそうなん？」→「ぶっちゃけ17/18がレアケースだった可能性がある」という流れできました。

これを踏まえて僕は「補強が必要。ただ財政的に補強資金に限りがあることを考えると先に補強すべきポジはあるように思うし。。。チャビ、大変そうだな。。。。」という感想になりました。
...
ただ今回色々調べてみて、ストップ能力が普通より下がデフォルトなのは流石に無視できない問題でしょう。「金あるの？、誰にするの？」と聞かれると困りますが、「交代するタイミング」は既に訪れてるように思いました。
